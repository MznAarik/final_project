...
<div class="tickets-container">
    <!-- Flash Messages -->
    @if (session('success'))
        <div style="position: fixed; top: 20px; right: 20px; padding: 1rem 1.5rem; border-radius: 6px; color: white; font-weight: 500; z-index: 9999; background: #059669;">
            {{ session('success') }}
        </div>
    @endif
    @if (session('error'))
        <div style="position: fixed; top: 20px; right: 20px; padding: 1rem 1.5rem; border-radius: 6px; color: white; font-weight: 500; z-index: 9999; background: #ef4444;">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="tickets-header">
        ...
        <div class="tickets-stats">
            ...
            <div class="stat-item">
                <div class="stat-number" id="totalRevenue">NPR {{ number_format($tickets->sum(function($ticket) {
                    return collect($ticket->payments)->sum('amount') * 133;
                }), 2) }}</div>
                            ...
                            <td>
                                <span class="status-badge status-{{ $ticket->status }}">
                                    @switch($ticket->status)
                                        ...
                                        @case('used') <i class="fa-solid fa-ticket-simple"></i> @break
                                    @endswitch
...
(function() {
    ...
    function applyFilters() {
        ...
        rows.forEach(row => {
            ...
            if (matchesSearch && matchesStatus && matchesEvent && matchesPayment) {
                ...
                const amountElement = row.querySelector('.payment-amount-usd');
                
                if (quantityElement) {
                    stats.quantity += parseInt(quantityElement.textContent) || 0;
                }
                if (amountElement) {
                    const usdAmount = parseFloat(amountElement.textContent.replace('(USD ', '').replace(')', '')) || 0;
                    stats.revenue += usdAmount * USD_TO_NPR_RATE;
                }
            } else {
    ...
    function exportTickets() {
        ...
        csvContent += "Ticket ID,User Name,User Email,Event Name,Event Date,Status,Payment Method,Payment Amount (NPR),Payment Amount (USD),Payment ID,Categories,Quantity\n";
        ...
        visibleRows.forEach(row => {
            ...
            const eventDate = row.querySelector('.event-date')?.textContent?.replace('📅 ', '').trim() || 'N/A';
            ...
            const paymentAmountNPR = row.querySelector('.payment-amount span:first-child')?.textContent?.trim() || 'NPR 0.00';
            const paymentAmountUSD = row.querySelector('.payment-amount-usd')?.textContent?.trim() || '(USD 0.00)';
            const paymentId = row.querySelector('.payment-id')?.textContent?.trim() || 'N/A';
